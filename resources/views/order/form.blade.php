                    ...
                    <div class="card-body">
                        <form action="{{ route('orders.update', $pivot->id) }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-control">
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="grade" id="grade_a" value="A"
                                               checked>
                                        <label class="form-check-label" for="grade_a">A</label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="grade" id="grade_b"
                                               value="B">
                                        <label class="form-check-label" for="grade_b">B</label>
                                    </div>
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input" type="radio" name="grade" id="grade_c"
                                               value="C">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-4">
                                    <input type="number" name="error" id="error" class="form-control" placeholder="0"
                                           max="{{ $pivot->batch->qty }}">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ url('orders/start/' . $pivot->id) }}" method="POST">
                            ...
                            <div class="form-group form-row">
                                <label for="qty" class="col-form-label col-12 col-md-3 text-right">Qty Setelah Error /
                                    Sisa</label>
                                <div class="col-12 col-md-6">
                                    <input type="number" name="qty" id="qty" class="form-control" min="0"
                                           value="{{ @$show || @$startOrder ? $pivot->qty_before_this_step - $pivot->current_step_processed : 1 }}" {{ @$show || @$startOrder ? 'readonly' : '' }}>
                                </div>
                            ...
                            @foreach($engines as $engine)
                                @if($engine->capacity - $engine->usage->where('active', TRUE)->sum('qty') > 0)
                                    <div class="form-group form-row align-items-center">
                                        <label for="process_qty_{{ $engine->id }}"
                                               class="col-form-label col-12 col-md-3 text-right">Mesin {{ $engine->name }}
                                            (Max. {{ $engine->capacity - $engine->usage->where('active', TRUE)->sum('qty') }}
                                            Kg)</label>
                                        <div class="col-12 col-md-3">
                                            <input type="number" name="process_qty[{{ $engine->id }}]"
                                                   id="process_qty_{{ $engine->id }}" class="form-control"
                                                   value="{{ old('process_qty.'.$engine->id) }}" placeholder="0"
                                                   max="{{ $engine->capacity - $engine->usage->where('active', TRUE)->sum('qty') }}">
                                        </div>
                                        <div class="col-12 col-md-3">
                                            <small class="text-muted">Kg</small>
                            ...
                            <div class="form-group row">
                                <div class="col-12 col-md-6 offset-3">
                                    <button type="submit"
                                            class="btn btn-primary" {{ $pivot->current_step_qty > 0 ? '' : 'disabled' }}>
                                        Mulai Produksi
                                    </button>
                                </div>
                            </div>
                    ...
                    <form action="{{ route('orders.store') }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="no_so" id="no_so" class="form-control"
                                   value="{{ @$show || @$startOrder ? @$pivot->batch->order->no_so : $no_so }}"
                                   readonly>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="sales_id" id="sales"
                                    @if(@$show || @$mulaiProduksi) disabled @endif>
                                <option value="" selected disabled>- Pilih Customer -</option>
                                @foreach($sales as $sales)
                                    <option value="{{ $sales->id }}"
                                            @if(@$show || (@$pivot->batch->order->sales->id == @$sales->id)) selected @endif>
                                        {{ $sales->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-control select2" name="item_id" id="items"
                                ...
                                @foreach($items as $item)
                                    <option value="{{ $item->id }}"
                                            @if(@$show || (@$pivot->batch->order->item->id == @$item->id)) selected @endif>
                                        {{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @if(@!$show && @!$mulaiProduksi)
                            <div class="form-row">
                                ...
                                <div class="col-10">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="batches[0][color]" id="colors"
                                                @if(@$show) disabled @endif>
                                            <option value="" selected disabled>- Pilih Warna -</option>
                                            @foreach($colors as $color)
                                                <option value="{{ $color->id }}"
                                                        @if(@$show && (@$pivot->batch->color->id == @$color->id)) selected @endif>
                                                    {{ $color->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group form-row align-items-center">
                                        ...
                                        <div class="col-9 col-lg-2">
                                            <input type="number" name="batches[0][qty]" id="qty" class="form-control"
                                                   min="0"
                                                   value="{{ @$show || @$startOrder ? $pivot->batch->qty : 1 }}" {{ @$show || @$startOrder ? 'disabled' : '' }}>
                                        </div>
                                        <div class="col-3">
                                            Kilogram
                                        </div>
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <button type="button" id="tambah-batch" class="btn btn-light btn-block float-right">
                                        <i class="fa fa-plus fa-sm"></i> Tambah Batch
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                @if(@$show)
                                    ...
                                    <div class="col-4">
                                        <input type="number" name="qty" id="qty" class="form-control" min="0"
                                               value="{{ @$show || @$startOrder ? $pivot->actual_qty : 1 }}" {{ @$show || @$startOrder ? 'disabled' : '' }}>
                                    </div>
                                @else
                                    <div class="col-12">
                                ...
                                <div class="col-10">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="batches[0][color]" id="colors" disabled>
                                            ...
                                            @foreach($colors as $color)
                                                <option value="{{ $color->id }}"
                                                        @if((@$pivot->batch->color->id == @$color->id)) selected @endif>
                                                    {{ $color->name }}
                                                </option>
                                            @endforeach
                                        </select>
